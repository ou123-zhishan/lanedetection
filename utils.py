...
def  my_ap_way(landetctect_index,pixelY,pixelX,roadWidth,left_fit):
    ...
    print('pxy',pixelX,pixelY)
    lane_Pkx = 2 * left_fit[0] * aimLaneP[0] + left_fit[1]
    print('lane_PKx',lane_Pkx)
    # lane_Pky = 2 * a2 * aimLaneP[1] + a1
    print('a2',a2,a1,left_fit[0],left_fit[1])
    ...
    if int(640-landetctect_index) < 0:
        if lane_Pkx > 0 :
            ...
            # if int(640-landetctect_index) < -90:
...
def   aimpoin(show_img,leftx,lefty,left_fit,landetctect_index):
    ...
    ap_line.update(ap)
    if ap_line.detected is not True:
        Ap = ap_line.best_ap
        ap0 = Ap[0]
        ap1 = Ap[1]
    ...
    if 500 <= int(landetctect_index) <=750 :
        steerAngle = 0
    else :
